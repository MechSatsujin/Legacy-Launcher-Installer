...
# Run bash ./installer.sh on terminal in order to install, do not run this as sudo.

if [ "$(id -u)" -eq 0 ]
then
    if [ -n "$SUDO_USER" ]
    then
        printf "Currently, this installer is only allowed to run on a normal user. Will be fixing this problem soon." >&2
        exit 1
    fi
fi
